    ...
    <section id="companies" ref={ref} className="relative wrapper py-20 md:py-28">
      ...
      >
        ...
        <div className="relative max-w-3xl mx-auto">
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-20 hidden md:block" viewBox="0 0 600 400">
          ...
          <div className="flex flex-wrap justify-center gap-4 relative">
            {companies.map((company, idx) => (
              <motion.div
                ...
                className="glass card-hover rounded-xl p-4 md:p-6 flex flex-col items-center justify-center gap-2 md:gap-3 text-center w-40 sm:w-48 md:w-56"
              >
                <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full bg-white/10 flex items-center justify-center text-white font-bold text-xs sm:text-sm md:text-base">
                  {company.initial}
                </div>
                <span className="text-white/80 text-[11px] sm:text-xs md:text-sm font-medium leading-tight text-center">
                  {company.name}
                </span>
              </motion.div>
            ))}
